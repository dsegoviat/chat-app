...
const hooks = {
  sandbox: {
    onSandboxReady: [
      {
        command:
          'mkdir -p "$CODEX_HOME" && cp /home/agent/codex-auth/auth.json "$CODEX_HOME/auth.json"',
      },
      {
        command:
          'if [ ! -d node_modules ]; then pnpm install --frozen-lockfile; else echo "node_modules already present, skipping install"; fi',
        timeoutMs: 300000,
      },
    ],
  },
